'use client'

import { useState, useEffect } from 'react'
import Link from 'next/link'
import { usePathname } from 'next/navigation'
import { Menu, X, ArrowLeft } from 'lucide-react'

import { Button } from '@/components/ui/button'
import { Sheet, SheetContent, SheetTrigger } from '@/components/ui/sheet'
import {
  PROJECT_NAV_STEPS,
  getWorkflowSteps,
  getSettingsStep,
  getActiveNavStep,
  type ProjectNavStep,
} from '@/lib/navigation/projectNavSchema'
import { navLinkClasses, navIconClasses } from '@/lib/nav/navLinkClasses'
import { cn } from '@/lib/utils'

interface ProjectNavProps {
  projectId: string
  projectTitle?: string | null
  subtitle?: string | null
  currentPath?: string
  stepState?: { [key in ProjectNavStep['id']]?: 'ready' | 'blocked' | 'complete' }
  className?: string
}

/**
 * Canonical Project Navigation Component
 * 
 * This is the ONLY left nav component for project pages.
 * 
 * Structure:
 * - Top utility: Back to Projects (link)
 * - Project context: Project title
 * - WORKFLOW: Decision, Competitors, Evidence, Scorecard, Appendix
 * - META: Settings (pinned at bottom)
 * 
 * Guarantees:
 * - All steps always visible (never conditionally hidden)
 * - Fixed order (never reordered)
 * - Settings pinned at bottom
 * - Max 24px horizontal gutter (enforced by shell)
 */
export function ProjectNav({
  projectId,
  projectTitle,
  subtitle,
  currentPath,
  stepState,
  className,
}: ProjectNavProps) {
  const pathname = usePathname()
  const [mobileMenuOpen, setMobileMenuOpen] = useState(false)

  // Close mobile menu when route changes
  useEffect(() => {
    setMobileMenuOpen(false)
  }, [pathname])

  // Close mobile menu on escape key
  useEffect(() => {
    const handleEscape = (e: KeyboardEvent) => {
      if (e.key === 'Escape') {
        setMobileMenuOpen(false)
      }
    }
    document.addEventListener('keydown', handleEscape)
    return () => document.removeEventListener('keydown', handleEscape)
  }, [])

  const activePath = currentPath ?? pathname
  const activeStepId = getActiveNavStep(activePath, projectId)
  const workflowSteps = getWorkflowSteps()
  const settingsStep = getSettingsStep()

  const renderNavLink = (step: ProjectNavStep) => {
    const href = step.href(projectId)
    const isActive = activeStepId === step.id
    const Icon = step.icon
    const stepStatus = stepState?.[step.id]
    const isDisabled = stepStatus === 'blocked'

    return (
      <Link
        key={step.id}
        href={href}
        prefetch
        data-testid={`project-nav-item-${step.id}`}
        className={navLinkClasses({ isActive, isDisabled, isDecision: step.id === 'decision' })}
        aria-current={isActive ? 'page' : undefined}
        aria-disabled={isDisabled}
        onClick={() => {
          if (process.env.NODE_ENV !== 'production') {
            console.log('[ProjectNav] Navigation click:', {
              stepId: step.id,
              href,
              currentPathname: activePath,
            })
          }
        }}
      >
        <Icon className={navIconClasses({ isActive, isDisabled })} />
        <span className="truncate">{step.label}</span>
        {step.id === 'decision' && isActive && (
          <span className="ml-auto text-[10px] font-medium text-blue-600/70" aria-label="Summary">
            Summary
          </span>
        )}
        {stepStatus && stepStatus !== 'ready' && (
          <span className="ml-auto text-[10px] font-medium text-slate-500" aria-label={stepStatus}>
            {stepStatus === 'blocked' ? 'Blocked' : stepStatus === 'complete' ? 'Complete' : ''}
          </span>
        )}
      </Link>
    )
  }

  const renderNavContent = () => (
    <>
      {/* Top Section: Back to Projects + Project Header */}
      <div className="flex-shrink-0">
        {/* Back to Projects Link */}
        <div className="p-3 border-b border-border">
          <Link
            href="/dashboard"
            prefetch
            className="flex items-center gap-2 text-xs font-medium text-slate-700 hover:text-slate-900 transition-colors"
            data-testid="back-to-projects-link"
            onClick={() => setMobileMenuOpen(false)}
          >
            <ArrowLeft className="h-3.5 w-3.5" />
            <span>Back to Projects</span>
          </Link>
        </div>

        {/* Project Header */}
        <div className="px-4 py-3 border-b border-border">
          <h2 className="text-sm font-semibold text-slate-900 leading-tight">
            {projectTitle || 'Untitled Project'}
          </h2>
          {subtitle && (
            <p className="text-xs text-slate-500 mt-1 line-clamp-2">
              {subtitle}
            </p>
          )}
        </div>
      </div>

      {/* Scrollable Navigation */}
      <nav className="flex-1 overflow-y-auto px-2.5 py-3" aria-label="Project navigation" data-testid="project-nav">
        {/* Workflow Steps */}
        <div className="space-y-0.5 mb-4">
          {workflowSteps.map(step => renderNavLink(step))}
        </div>
      </nav>

      {/* Sticky Settings Footer */}
      {settingsStep && (
        <div className="flex-shrink-0 border-t border-border bg-background">
          <div className="px-2.5 py-2.5">
            {renderNavLink(settingsStep)}
          </div>
        </div>
      )}
    </>
  )

  return (
    <>
      {/* Desktop Sidebar */}
      <aside
        className={cn(
          'hidden md:flex md:flex-col md:fixed md:inset-y-0 md:pt-[57px] md:border-r md:border-border md:z-10 md:bg-background plinth-surface md:w-[240px]',
          className
        )}
      >
        <div className="flex flex-col h-full">
          {renderNavContent()}
        </div>
      </aside>

      {/* Mobile Header */}
      <div className="md:hidden fixed top-[57px] left-0 right-0 z-40 border-b border-border plinth-surface bg-background">
        <div className="flex items-center gap-3 px-4 py-2.5">
          <Link
            href="/dashboard"
            prefetch
            className="flex items-center justify-center h-8 w-8 rounded-md text-slate-700 hover:bg-slate-50 hover:text-slate-900 transition-colors"
            aria-label="Back to Projects"
          >
            <ArrowLeft className="h-4 w-4" />
          </Link>
          <div className="flex-1 min-w-0">
            <h2 className="text-sm font-semibold text-slate-900 truncate">
              {projectTitle || 'Untitled Project'}
            </h2>
            {subtitle && (
              <p className="text-xs text-slate-500 truncate">
                {subtitle}
              </p>
            )}
          </div>
          <Sheet open={mobileMenuOpen} onOpenChange={setMobileMenuOpen}>
            <SheetTrigger asChild>
              <Button
                type="button"
                variant="ghost"
                size="icon"
                className="h-8 w-8"
                aria-label="Toggle menu"
                aria-expanded={mobileMenuOpen}
              >
                {mobileMenuOpen ? (
                  <X className="h-5 w-5" />
                ) : (
                  <Menu className="h-5 w-5" />
                )}
              </Button>
            </SheetTrigger>
            <SheetContent side="left" className="w-64 p-0">
              <div className="flex flex-col h-full">
                {renderNavContent()}
              </div>
            </SheetContent>
          </Sheet>
        </div>
      </div>
    </>
  )
}

